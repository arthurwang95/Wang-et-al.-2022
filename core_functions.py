...
def detect_upper_outliers(cells_average_gfp_list):
    ...
    # Non-trivial logic selecting the regression basis  #change 10 to 100 on July 26,2021

    not_nan_mask = np.logical_not(np.isnan(cells_average_gfp_list))
    # print 'debug: not_nan_mask', not_nan_mask

    cells_average_gfp_list_f = np.array(cells_average_gfp_list)[not_nan_mask].tolist()
    cell_no_f = np.array(cell_no)[not_nan_mask].tolist()

    regression_base = min(len(cells_average_gfp_list_f) - 3, 100)

    # print 'debug: regression base', regression_base
    # print 'debug: cells regressed', np.array(cells_average_gfp_list_f)[1:regression_base]

    slope, intercept, _, _, _ = stats.linregress(np.array(cell_no_f)[1:regression_base],
                                                 np.array(cells_average_gfp_list_f)[
                                                 1:regression_base])




    reg_base_deviation = np.array(cells_average_gfp_list) - intercept - slope * np.array(cell_no)

    std_ = np.nanstd(reg_base_deviation)
    ...
    # print 'debug: predicted_average_gfp', predicted_average_gfp
    # print 'debug: std_err', std_err
    non_outliers = arg_sort[np.array(cell_no)[np.array(predicted_average_gfp + std_err) >
                                              np.array(cells_average_gfp_list)]]
    # print 'debug: non_outliers', non_outliers
