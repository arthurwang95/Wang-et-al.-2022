...
def Linhao_traverse(main_root,
    ...
    # create a new dict = defaultdict(['', '']) i.e. if no key is found, return ['','']
    tags_dict = defaultdict(lambda: [])
    # similar to matched_images
    if matching_rule:
        assert(matching_map is not None)

    for current_location, sub_directories, files in os.walk(main_root):

        if files: # into each file

            for img in files:
                if ('.TIF' in img or '.tif' in img) and '_thumb_' not in img and 'w3Brightfield' not in img:

                    prefix = cf.split_and_trim(current_location, main_root) #extract ['BeforeHS'] or ['HS']

                    img_codename = img.split(' ')[0].split('_')
                    # e.g. 'AfterHS_GPM1_a_01_w1488 Laser.TIF' --> ['AfterHS', 'GPM1', 'a', '01', 'w1488']
                    color = matching_map[img_codename[-1]]  # extract 'w1488' or 'w2561'
                    name_pattern = ' - '.join(prefix + img_codename[:-1])
                    # e.g. 'HS - AfterHS - GPM1 - a - 01'
                    matched_images[name_pattern][color] = os.path.join(current_location, img)
                    # i.e. the full path 'C:\\Users\\83849\\Desktop\\Analysis\\data\\HS\\AfterHS_GPM1_a_01_w1488 Laser.TIF'
                    time_stamp = prefix[-1]

                    if time_stamp == 'HS':
                        time = 0
                    elif 'HS' in time_stamp:  # i.e 'BeforeHS'
                        time = -30
                    else:   # not used for two time point data
